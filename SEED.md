...
- **Максимальная плотность сессий:** Агенты работают не в одной итерации, а в одной непрерывной сессии, генерируя тысячи строк кода за раз, редактируя и создавая множество документов и модулей.
- **Многоуровневое планирование:** Meta-агент должен думать наперёд, строить roadmap и ставить задачи на много итераций вперёд, эффективно комбинируя стратегические и тактические задачи.
- **Глобальная эффективность:** Universal-агент должен комбинировать роли и задачи, активно использовать готовые MVP и открытые решения на GitHub, а также проводить комплексные интернет-исследования при нехватке локальных данных.
...
- **Супер-плотная сессия (Macro-Session):** За один запуск агенты обязаны выполнять масштабные блоки работы (тысячи строк кода, создание множества файлов) в единой сессии, состоящей из нескольких итераций. Любые "микро-шаги" строго запрещены.
