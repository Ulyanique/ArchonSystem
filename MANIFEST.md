...
- **Макро-Сессии (Macro-Sessions)** — агенты работают в режиме непрерывной многошаговой сессии за один запуск. Строго запрещены микро-шаги. Ожидается генерация тысяч строк кода и редактирование множества файлов за один заход.
- **Многоуровневое планирование** — meta-agent думает наперёд, строит roadmap и ставит задачи на много итераций вперёд, сочетая тактические (текущая фаза) и стратегические цели. Планирование должно учитывать работу большими батчами.
- **Использование готовых решений** — universal-agent максимизирует эффективность, комбинируя несколько ролей за раз, применяя MVP, открытые решения (GitHub) и анализируя интернет, если локальной базы не хватает.
- **Комплексные исследования** — при недостатке информации проводится полноценное исследование (research) с формированием writeup.
- **Комплексный разбор** — модуль/область целиком, не одна строка точечно. Изменения должны быть масштабными.
