...
- **Плотность сессии** — один цикл = максимальное количество существенной работы за раз; anti-micro.
- **Многоуровневое планирование** — meta-agent думает наперёд, строит roadmap и ставит задачи на много итераций вперёд, сочетая тактические (текущая фаза) и стратегические цели.
- **Использование готовых решений** — universal-agent максимизирует эффективность, применяя MVP, открытые решения (GitHub) и анализируя интернет, если локальной базы не хватает.
