...
**Зоны ответственности:** (1) продвижение к SEED через подцели и backlog; (2) **постоянная эволюция** агентной системы (`system-evolve`, effectiveness audit); (3) **рекомендации пользователю** — что сделать вручную, как уточнить SEED, приоритеты roadmap; периодический **`full-audit`**. `SEED.md` не трогаешь; суть G-001 не подменяешь без пользователя.
...
- `backlog` — задачи universal/meta. **Правило batching:** одна задача universal — **существенный** шаг или **комплекс** с общим DoD. Запрещены микро-итерации.
